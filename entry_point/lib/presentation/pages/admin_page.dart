...
Future<void> _downloadReport(BuildContext context, WidgetRef ref) async {
  ...
  try {
    ...
    if (_isMobile) {
      ...
      // Веб: браузер сам предложит диалог сохранения
      ...
      // Десктоп: диалог "Сохранить как" с выбором пути
      ...
      if (outputPath == null) {
        ...
        if (context.mounted) showInfoSnack(context, 'Сохранение отменено');
        return;
      }
      final file = File(outputPath);
      await file.writeAsBytes(uint8Bytes);
      if (context.mounted) {
        showSuccessSnack(context, 'Отчёт сохранён в $outputPath');
      }
    }
  } catch (e) {
    if (context.mounted) {
          ...
          child: Column(
            ...
            children: [
              ...
              _AdminActionButton(
                ...
                label: 'Временные пропуска',
                subtitle: 'Гости, подрядчики, курьеры',
                color: Theme.of(context).colorScheme.primary,
                onTap: () => context.push('/guest-passes'),
              ),
              const SizedBox(height: 16),
              _AdminActionButton(
